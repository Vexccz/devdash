...
### Automations *(new in v0.8.0)*
- Cron-scheduled jobs per project: auto git pull or auto redeploy
- 6 preset schedules (30m, hourly, every 4h, daily 9am/9pm, weekly Mon 9am)
- Run history with success/failure log per job (SQLite-backed)
- Enable/disable toggle, inline validation of cron expressions

### Database Health *(new in v0.8.0)*
- Ping MongoDB (`admin.serverStatus`) and Postgres (`SELECT version()`) targets
- Shows latency, server version, last-checked timestamp
- **Auto-detect from env**: scans `.env` files for `MONGODB_URI`, `DATABASE_URL`, `POSTGRES_URL` and imports matching targets
- Connection strings stored locally in config.json; passwords masked in UI

### Metrics *(new in v0.8.0)*
- **Render**: CPU + memory sparklines over 1h / 6h / 24h / 3d windows
- **Vercel**: visitor/pageview summary + daily trend + top pages (when Web Analytics is enabled on the project)

### Time
- Active project auto-tracked from the currently-open project detail
- Idle timeout (default 2 min) stops the timer on inactivity
- Per-day and per-week totals

### Deps
- Outdated package count per project (major / minor / patch)
- Weekly auto-run on Monday 9 AM or manual refresh

### Chat *(new in v0.4.0, polished v0.5.0-v0.5.1)*
- Ollama streaming chat (`/api/chat` NDJSON) with chat history persisted in SQLite
- Model picker, temperature slider, system prompt
- Markdown rendering with `marked` + `DOMPurify`, code block syntax highlighting (`highlight.js`), copy buttons
- Ctrl+Enter to send, Stop to abort streaming
- Supports Ollama Cloud via optional API key + custom base URL

### Settings
- Vercel + Render tokens with inline Test buttons
- Ollama base URL, default model, system prompt, temperature, optional API key
- Theme picker (Dark / Light / System)
- Encrypted config export/import (AES-256-GCM with scrypt passphrase)
- Re-run onboarding wizard anytime
- Auto-launch on Windows startup

## Onboarding wizard *(new in v0.9.1)*

First launch walks through four quick steps:
1. Welcome
2. Paste Vercel/Render tokens with inline test buttons
3. Smart scan: pick a parent folder, auto-select repos with matching deploys
4. Preferences: theme, auto-launch, poll interval

Re-run anytime from Settings → About → "Re-run setup wizard".

## First deploy *(new in v0.10.0)*

For projects without a deploy provider configured, a **🚀 Deploy** button opens a modal that:
1. Pre-fills framework, build command, and output directory based on detected project type
2. Imports environment variables from your local `.env`
3. Creates the Vercel project (`/v10/projects`) or Render service (`/v1/services`) via official APIs
4. Attaches env vars, triggers the first build
5. Updates local DevDash config with the new deploy ID and live URL

Supports Vercel frameworks (Vite, Next.js, React, Electron, Flutter, Node) and Render service types (static site, web service) with region and plan selection.
